...
public:
  Tracker(){
    nh.param("sub_img_name", this->sub_img_name, std::string("/camera/rgb/image_raw"));
    ...
    this->sub_img = nh.subscribe(this->sub_img_name, 100, &Tracker::sub_img_CB, this);
    this->sub_bbox = nh.subscribe(this->target_boundingBox_name, 10, &Tracker::sub_bbox_CB, this);
    this->pub_bbox = nh.advertise<darknet_dnn::BoundingBox>("/result_bbox", 1);
    this->pub_result = nh.advertise<std_msgs::Bool>("/result_bool", 1);

    image_transport::ImageTransport it(this->nh);
    this->pub_img = it.advertise("/result_img", 1);
  }
